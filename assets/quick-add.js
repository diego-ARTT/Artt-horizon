...
export class QuickAddComponent extends Component {
  ...
  #updateVariantPicker(newHtml) {
    ...
    // `variant-picker` is lazy-loaded, so the element can be in the DOM before it has
    // upgraded and exposed `updateVariantPicker`. Gate on the upgraded class, not on
    // mere existence, or this throws "is not a function".
...
class QuickAddDialog extends DialogComponent {
  ...
  handleCartUpdate = event => {
    event.promise
      ...
      .catch(error => {
        if (error?.name !== 'AbortError')
          console.warn('[quick-add] Event promise rejected:', error);
      });
  };
    ...
    event.promise
      .then(({ detail }) => {
        ...
        const anchorElement = /** @type {HTMLAnchorElement} */ (
          html.querySelector('.view-product-title a')
        );
