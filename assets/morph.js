...
function walk(newNode, oldNode, options) {
  ...
  if (newType === 1 /* ELEMENT_NODE */) {
    ...
    if (
      oldEl.hasAttribute('data-skip-node-update') &&
      newEl.hasAttribute('data-skip-node-update')
    ) {
...
function skipsNodeUpdate(newNode, oldNode) {
  return (
    newNode.hasAttribute('data-skip-node-update') && oldNode.hasAttribute('data-skip-node-update')
  );
}
...
function same(a, b, options) {
  ...
  if (aType === Node.COMMENT_NODE && bType === Node.COMMENT_NODE)
