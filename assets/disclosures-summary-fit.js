...
class DisclosuresSummaryFit extends Component {
  ...
  get #summaryItem() {
    const summaryItem = this.querySelector(
      '.disclosures__summary-content > .disclosures__summary-item'
    );
  ...
  #connectObservers() {
    ...
    for (const image of this.querySelectorAll('.disclosures__summary-content .details__icon')) {
      image.addEventListener('load', this.#scheduleMeasure, {
  ...
  #measure() {
    ...
    const nextLayout =
      requiredWidth <= availableWidth + WIDTH_TOLERANCE ? INLINE_LAYOUT : STACKED_LAYOUT;
